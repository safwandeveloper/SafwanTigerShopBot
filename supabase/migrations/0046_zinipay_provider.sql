-- ZiniPay hosted bKash invoice top-ups.
...
create or replace function public.credit_zinipay_deposit(
    p_deposit_id bigint,
    p_tx_hash text
)
returns table (
...
begin
    update public.deposits as dep
     ...
     where dep.id = p_deposit_id
       ...
       and not exists (
           select 1 from public.deposits as other_dep
            where other_dep.tx_hash = p_tx_hash and other_dep.id <> p_deposit_id
       )
    returning dep.user_id, dep.amount into v_user_id, v_amount;

    if not found then
        select dep.user_id, dep.amount into v_user_id, v_amount
          from public.deposits as dep
         where dep.id = p_deposit_id and dep.status = 'approved';
        if found then
            select wallet_user.balance into v_balance
    ...
    update public.users as wallet_user
     ...
     where wallet_user.telegram_id = v_user_id
    returning wallet_user.balance into v_balance;
